...
FROZEN_BATCH_PROVENANCE_FILE_SHA256 = (
    "91aebc8bf391a7d920ec2a99772bfc3e44567285439bb3e61085e594a80be6d9"
)
FROZEN_CANONICAL_REPRODUCER_FILE_SHA256 = (
    "b0b375388eab88412edfea695196f387f122d26d98f85979fc86758c3e95606f"
)
FROZEN_STANDALONE_GENERATOR_FILE_SHA256 = (
    "ff634d508e05b45e2ce53ad706b85b52408d8a0955d534013e7f725657428ae1"
)
FROZEN_DISCOVERY_ORDER_MASKS_SHA256 = (
...
FROZEN_CORE_REPLAY_SCHEMA = (
    "ramsey-r3-18-n100-exact-budget7-branch1-core-proof-replay-v1"
)
FROZEN_CORE_REPLAY_STATUS = "ALL_FINAL_BRANCH1_CORE_PROOFS_REPLAYED"
...
UNSAT_PROMOTION_SCHEMA = (
    "ramsey-r3-18-n100-exact-budget7-branch1-cegar-unsat-promotion-v1"
)
AUGMENTED_PROOF_REPLAY_SCHEMA = (
    "ramsey-r3-18-n100-exact-budget7-branch1-cegar-augmented-proof-replay-v1"
)
ENDPOINT_RECORD_SCHEMA = (
    "ramsey-r3-18-n100-exact-budget7-branch1-cegar-endpoint-record-v1"
)
...
class StableOutputDirectory:
    """A newly created output directory anchored by an open directory fd."""

    directory_fd: int
    display_path: Path

    @classmethod
    def create(cls, path: Path) -> "StableOutputDirectory":
        basename = path.name
        if not basename or basename in {".", ".."}:
            raise AuditError("output directory has no safe basename")
        try:
            path.parent.mkdir(parents=True, exist_ok=True)
            parent = path.parent.resolve(strict=True)
        except OSError as error:
            raise AuditError("output parent could not be prepared") from error
        flags = os.O_RDONLY
        ...
        try:
            ...
            raise AuditError("output parent could not be opened safely") from error
        try:
            ...
            if (
                ...
                raise AuditError("output parent changed during preflight")
            try:
                os.mkdir(basename, mode=0o700, dir_fd=parent_fd)
            except FileExistsError as error:
                raise FileExistsError(
                    f"refusing existing output directory: {basename}"
                ) from error
            directory_fd = os.open(basename, flags, dir_fd=parent_fd)
            opened_directory = os.fstat(directory_fd)
            if not stat.S_ISDIR(opened_directory.st_mode):
                os.close(directory_fd)
                raise AuditError("new output entry is not a directory")
        except Exception:
            os.close(parent_fd)
            raise
        os.close(parent_fd)
        return cls(directory_fd=directory_fd, display_path=parent / basename)
    ...
    def _install_bytes(
        ...
        *,
        ...
        if self.directory_fd < 0:
            raise AuditError("output directory is already closed")
        if not basename or Path(basename).name != basename:
            raise AuditError("output artifact has an unsafe basename")
        temporary = ""
        descriptor = -1
        ...
        flags |= getattr(os, "O_NOFOLLOW", 0)
        for _ in range(128):
            candidate = f".{basename}.{os.getpid()}.{secrets.token_hex(12)}.tmp"
            try:
                descriptor = os.open(
                ...
                raise AuditError("output temporary file could not be created") from error
            temporary = candidate
            break
        if descriptor < 0:
            raise AuditError("could not allocate a private output file")
        ...
        try:
            for block in chunks:
                if not isinstance(block, bytes):
                    raise AuditError("output stream yielded a non-byte block")
                digest.update(block)
                size += len(block)
                offset = 0
                while offset < len(block):
                    try:
                        ...
                        continue
                    if written <= 0:
                        raise AuditError("short write while creating output artifact")
                    offset += written
            actual_sha256 = digest.hexdigest()
            if expected_bytes is not None and size != expected_bytes:
                raise AuditError("output artifact byte count changed during install")
            if expected_sha256 is not None and actual_sha256 != expected_sha256:
                raise AuditError("output artifact SHA-256 changed during install")
            os.fsync(descriptor)
            os.close(descriptor)
            descriptor = -1
            try:
                ...
                if error.errno == errno.EEXIST:
                    raise FileExistsError(f"refusing to overwrite {basename}") from error
                raise
            os.unlink(temporary, dir_fd=self.directory_fd)
            installed = True
            os.fsync(self.directory_fd)
            return {"basename": basename, "bytes": size, "sha256": actual_sha256}
        except (AuditError, FileExistsError):
            raise
        except OSError as error:
            raise AuditError("output artifact could not be installed atomically") from error
        finally:
            if descriptor >= 0:
                os.close(descriptor)
    ...
    def write_json(self, basename: str, payload: dict[str, Any]) -> dict[str, Any]:
        rendered = (json.dumps(payload, indent=2, sort_keys=True) + "\n").encode(
            "utf-8"
        )
        return self._install_bytes(basename, (rendered,))
...
def validate_generation_provenance(
    ...
    canonical_reproducer_path: Path,
) -> dict[str, Any]:
    """Bind the canonical tracked reproducer to the original run snapshot."""

    try:
        raw = provenance_path.read_bytes()
    except OSError as error:
        raise AuditError("A+ generation provenance is unreadable") from error
    if hashlib.sha256(raw).hexdigest() != FROZEN_BATCH_PROVENANCE_FILE_SHA256:
        raise AuditError("A+ generation provenance SHA-256 mismatch")
    if (
        _sha256_file(canonical_reproducer_path)
        != FROZEN_CANONICAL_REPRODUCER_FILE_SHA256
    ):
        raise AuditError("canonical A+ reproducer SHA-256 mismatch")
    payload = _strict_json_bytes(raw, "A+ generation provenance")
    if payload.get("schema") != (
        "ramsey-r3-18-n100-exact-budget7-branch1-cegar-Aplus-provenance-v1"
    ):
        raise AuditError("A+ generation provenance schema mismatch")
    if payload.get("status") != "FROZEN_FILTERED_I18_BATCH_VERIFIED":
        raise AuditError("A+ generation provenance status mismatch")
    masks, width = _canonical_mask_array(
        payload.get("masks"),
        description="A+ provenance masks",
        order=PRODUCTION_ORDER,
        set_size=PRODUCTION_BANK_SET_SIZE,
        expected_count=MASK_BATCH_SIZE,
    )
    if ordered_masks_sha256(masks, width) != FROZEN_MASK_BATCH_ORDERED_SHA256:
        raise AuditError("A+ provenance masks differ from the frozen batch")
    if payload.get("ordered_masks_sha256") != FROZEN_MASK_BATCH_ORDERED_SHA256:
        raise AuditError("A+ provenance stored-order digest mismatch")
    if (
        payload.get("discovery_order_masks_sha256")
        != FROZEN_DISCOVERY_ORDER_MASKS_SHA256
    ):
        raise AuditError("A+ provenance discovery-order digest mismatch")

    canonical_batch = payload.get("canonical_gate_batch")
    if not isinstance(canonical_batch, dict) or canonical_batch != {
        "path": "r3_18_budget7_branch1_cegar_Aplus_batch.json",
        "file_sha256": FROZEN_MASK_BATCH_FILE_SHA256,
        "schema": MASK_BATCH_SCHEMA,
    }:
        raise AuditError("A+ provenance canonical-batch binding mismatch")
    history = payload.get("history_exclusion")
    if (
        not isinstance(history, dict)
        or history.get("file_sha256") != FROZEN_HISTORY_EXCLUSION_FILE_SHA256
        or history.get("masks_count") != HISTORY_EXCLUSION_MASKS
        or history.get("ordered_masks_sha256")
        != HISTORY_EXCLUSION_ORDERED_SHA256
    ):
        raise AuditError("A+ provenance history binding mismatch")
    fixed = payload.get("fixed_base_exclusion")
    if (
        not isinstance(fixed, dict)
        or fixed.get("masks_count") != FIXED_BRANCH1_BASE_MASKS
        or fixed.get("ordered_masks_sha256")
        != FIXED_BRANCH1_BASE_ORDERED_SHA256
    ):
        raise AuditError("A+ provenance fixed-base binding mismatch")
    enumeration = payload.get("enumeration")
    if (
        not isinstance(enumeration, dict)
        or enumeration.get("strategy") != "reverse-first"
        or enumeration.get("completed_witnesses_scanned") != 6_512
        or enumeration.get("excluded_witnesses") != 2_416
        or enumeration.get("accepted_witnesses") != MASK_BATCH_SIZE
        or enumeration.get("recursive_nodes") != 192_166
    ):
        raise AuditError("A+ provenance enumeration ledger mismatch")
    original = payload.get("provenance")
    if (
        not isinstance(original, dict)
        or original.get("generator_script_sha256")
        != FROZEN_STANDALONE_GENERATOR_FILE_SHA256
        or original.get("solver_invoked") is not False
    ):
        raise AuditError("A+ provenance standalone-generator binding mismatch")

    return {
        "provenance_basename": provenance_path.name,
        "provenance_sha256": FROZEN_BATCH_PROVENANCE_FILE_SHA256,
        "canonical_reproducer": {
            "basename": canonical_reproducer_path.name,
            "sha256": FROZEN_CANONICAL_REPRODUCER_FILE_SHA256,
            "normative": True,
            "reproduces_batch_sha256": FROZEN_MASK_BATCH_FILE_SHA256,
        },
        "original_standalone_snapshot": {
            "basename": original.get("generator_script"),
            "sha256": FROZEN_STANDALONE_GENERATOR_FILE_SHA256,
            "normative": False,
            "relationship": (
                "original Sirius generation snapshot recorded by the frozen "
                "provenance sidecar; the tracked canonical reproducer is the "
                "publication source of truth"
            ),
        },
        "solver_invoked_during_batch_generation": False,
        "discovery_order_masks_sha256": FROZEN_DISCOVERY_ORDER_MASKS_SHA256,
    }
...
def evaluate_dimacs_model(
    ...
    *,
    ...
    actual_sha256 = digest.hexdigest()
    if expected_sha256 is not None and actual_sha256 != expected_sha256:
        raise AuditError("augmented CNF SHA-256 changed during model evaluation")
    return {
        "all_clauses_satisfied": True,
        "clauses_evaluated": checked,
        "cnf_sha256_evaluated": actual_sha256,
    }
...
def pending_unsat_promotion_record() -> dict[str, Any]:
    """Describe the evidence still required before accepting scoped UNSAT."""

    return {
        "schema": UNSAT_PROMOTION_SCHEMA,
        "status": "BLOCKED_PENDING_INDEPENDENT_PROOF_REPLAYS",
        "automatic_promotion": False,
        "required_singleton_core_replay": {
            "schema": FROZEN_CORE_REPLAY_SCHEMA,
            "status": FROZEN_CORE_REPLAY_STATUS,
            "summary_sha256": FROZEN_SINGLETON_SUMMARY_SHA256,
            "exact_labels": ["K_a", "K_b", "K_c", "K_d"],
            "records_verified": 4,
            "assets_verified": 8,
        },
        "required_augmented_proof_replay": {
            "schema": AUGMENTED_PROOF_REPLAY_SCHEMA,
            "status": "AUGMENTED_CNF_PROOF_REPLAYED",
            "cnf_sha256": FROZEN_AUGMENTED_CNF_SHA256,
            "drat_trim_source_commit": PINNED_DRAT_TRIM_COMMIT,
            "exact_verified_line": "s VERIFIED",
        },
...
def validate_unsat_promotion_evidence(
    gate_result: dict[str, Any],
    core_replay: dict[str, Any],
    augmented_replay: dict[str, Any],
) -> dict[str, Any]:
    """Bind fresh singleton and augmented DRAT replays to one UNSAT record."""

    solver = gate_result.get("solver")
    augmentation = gate_result.get("augmentation")
    if not isinstance(solver, dict) or solver.get("status") != STATUS_UNSAT_UNCHECKED:
        raise AuditError("UNSAT promotion requires an unchecked generated proof")
    if not isinstance(augmentation, dict):
        raise AuditError("UNSAT promotion lacks an augmentation record")
    cnf = augmentation.get("augmented_cnf")
    proof = solver.get("proof")
    if (
        not isinstance(cnf, dict)
        or cnf.get("sha256") != FROZEN_AUGMENTED_CNF_SHA256
        or cnf.get("bytes") != FROZEN_AUGMENTED_CNF_BYTES
        or cnf.get("clauses") != FROZEN_AUGMENTED_CNF_CLAUSES
        or not isinstance(proof, dict)
        or not _SHA256_RE.fullmatch(str(proof.get("sha256", "")))
    ):
        raise AuditError("UNSAT promotion formula/proof identity mismatch")

    if (
        core_replay.get("schema") != FROZEN_CORE_REPLAY_SCHEMA
        or core_replay.get("status") != FROZEN_CORE_REPLAY_STATUS
        or core_replay.get("records_verified") != 4
        or core_replay.get("assets_verified") != 8
    ):
        raise AuditError("UNSAT promotion lacks the exact four-core replay")
    core_summary = core_replay.get("summary")
    records = core_replay.get("records")
    core_checker = core_replay.get("checker")
    if (
        not isinstance(core_summary, dict)
        or core_summary.get("sha256") != FROZEN_SINGLETON_SUMMARY_SHA256
        or not isinstance(core_checker, dict)
        or core_checker.get("source_commit") != PINNED_DRAT_TRIM_COMMIT
        or core_checker.get("sha256") not in PINNED_DRAT_TRIM_SHA256
        or not isinstance(records, list)
        or [record.get("label") for record in records if isinstance(record, dict)]
        != ["K_a", "K_b", "K_c", "K_d"]
        or any(
            not isinstance(record, dict)
            or record.get("semantic_reconstruction")
            != "VERIFIED_INDEPENDENT_CNF_RECONSTRUCTION"
            or not isinstance(record.get("drat_replay"), dict)
            or record["drat_replay"].get("status") != "VERIFIED"
            or record["drat_replay"].get("exitcode") != 0
            for record in records
        )
    ):
        raise AuditError("UNSAT promotion singleton replay binding mismatch")

    replay_cnf = augmented_replay.get("augmented_cnf")
    replay_proof = augmented_replay.get("proof")
    replay_checker = augmented_replay.get("checker")
    if (
        augmented_replay.get("schema") != AUGMENTED_PROOF_REPLAY_SCHEMA
        or augmented_replay.get("status") != "AUGMENTED_CNF_PROOF_REPLAYED"
        or augmented_replay.get("proof_verified") is not True
        or not isinstance(replay_cnf, dict)
        or replay_cnf.get("sha256") != cnf["sha256"]
        or replay_cnf.get("bytes") != cnf["bytes"]
        or not isinstance(replay_proof, dict)
        or replay_proof.get("gzip_sha256") != proof["sha256"]
        or not isinstance(replay_checker, dict)
        or replay_checker.get("source_commit") != PINNED_DRAT_TRIM_COMMIT
        or replay_checker.get("sha256") not in PINNED_DRAT_TRIM_SHA256
        or replay_checker.get("exitcode") != 0
        or replay_checker.get("verified_lines") != ["s VERIFIED"]
    ):
        raise AuditError("UNSAT promotion augmented-proof replay mismatch")

    return {
        "schema": UNSAT_PROMOTION_SCHEMA,
        "status": "SCOPED_UNSAT_EVIDENCE_BOUND_AFTER_FRESH_REPLAYS",
        "automatic_promotion": False,
        "augmented_cnf_sha256": cnf["sha256"],
        "proof_gzip_sha256": proof["sha256"],
        "singleton_summary_sha256": FROZEN_SINGLETON_SUMMARY_SHA256,
        "singleton_labels_replayed": ["K_a", "K_b", "K_c", "K_d"],
        "scoped_unsat_claim_accepted": True,
        "exact_seven_repair_exists": None,
        "global_ramsey_implication": None,
    }


def build_endpoint_record(
    solver: dict[str, Any], cnf_info: dict[str, Any]
) -> dict[str, Any]:
    """Create the endpoint-specific freeze ledger for SAT/UNSAT/UNKNOWN."""

    if (
        cnf_info.get("sha256") != FROZEN_AUGMENTED_CNF_SHA256
        or cnf_info.get("bytes") != FROZEN_AUGMENTED_CNF_BYTES
        or cnf_info.get("clauses") != FROZEN_AUGMENTED_CNF_CLAUSES
    ):
        raise AuditError("endpoint record has the wrong augmented CNF identity")
    status = solver.get("status")
    if not isinstance(status, str):
        raise AuditError("endpoint record lacks a solver status")
    record: dict[str, Any] = {
        "schema": ENDPOINT_RECORD_SCHEMA,
        "status": status,
        "augmented_cnf": {
        ...
        "solver_record_canonical_sha256": _canonical_json_sha256(solver),
        "files_to_freeze": ["branch1_cegar_augmented.cnf", "branch1_cegar_gate.json"],
        "learned_masks": [],
    }
    if status in {STATUS_SAT_WITNESS, STATUS_SAT_ESCALATE}:
        model = solver.get("model")
        evaluation = solver.get("model_evaluation")
        graph_audit = solver.get("I18_search")
        if (
            not isinstance(model, dict)
            or not _SHA256_RE.fullmatch(str(model.get("sha256", "")))
            or not isinstance(evaluation, dict)
            or evaluation.get("all_clauses_satisfied") is not True
            or evaluation.get("cnf_sha256_evaluated")
            != FROZEN_AUGMENTED_CNF_SHA256
            or not isinstance(graph_audit, dict)
        ):
            raise AuditError("SAT endpoint lacks its model/clause/graph audit")
        record["files_to_freeze"].append("branch1_cegar_sat.model.gz")
        record["sat_model"] = model
        record["independent_clause_audit"] = evaluation
        record["independent_graph_audit"] = graph_audit
        if status == STATUS_SAT_ESCALATE:
            if graph_audit.get("independent_set_exists") is not False:
                raise AuditError("SAT target escalation still has an I18 witness")
            record["candidate_target_implication"] = {
                "status": "CANDIDATE_REQUIRES_INDEPENDENT_PUBLICATION_PROMOTION",
                "conditional_result": "R(3,18) >= 101",
                "eligible_for_global_lower_bound": True,
                "local_branch_closure_only": False,
            }
    elif status == STATUS_UNSAT_UNCHECKED:
        proof = solver.get("proof")
        if not isinstance(proof, dict) or not _SHA256_RE.fullmatch(
            str(proof.get("sha256", ""))
        ):
            raise AuditError("UNSAT endpoint lacks its generated proof identity")
        record["files_to_freeze"].append("branch1_cegar_unsat.drat.gz")
        record["generated_proof"] = proof
        record["promotion"] = pending_unsat_promotion_record()
    elif status.startswith("UNKNOWN_"):
        if not isinstance(solver.get("checker"), dict):
            raise AuditError("UNKNOWN endpoint lacks its authenticated checker record")
        record["unknown_claim_boundary"] = (
            "Authenticated endpoint only: no cut learned and no SAT/UNSAT claim."
        )
    elif status == "NOT_RUN":
        record["not_run"] = True
    else:
        raise AuditError("endpoint record has an unrecognized state")
    return record
...
def run_gate(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="ramsey-cegar-inputs-") as temporary:
        ...
        staged_matrix = private / "seed.txt"
        staged_bank = private / "universal-bank.json"
        staged_masks = private / "Aplus-batch.json"
        staged_history = private / "history-exclusion.json"
        staged_provenance = private / "Aplus-provenance.json"
        staged_reproducer = private / "generate-Aplus.py"
        staged_runner = private / "cegar-gate.py"
        ...
        matrix_input = _stage_exact(
            matrix_path, staged_matrix, EXPECTED_INPUT_SHA256
        )
        bank_input = _stage_exact(bank_path, staged_bank, EXPECTED_BANK_SHA256)
        _stage_exact(
            masks_path, staged_masks, FROZEN_MASK_BATCH_FILE_SHA256
        )
        _stage_exact(
        ...
        _stage_exact(
            canonical_reproducer_path,
            staged_reproducer,
            FROZEN_CANONICAL_REPRODUCER_FILE_SHA256,
        )
        running_script = Path(__file__).resolve()
        runner_input = _stage_exact(
            running_script, staged_runner, _sha256_file(running_script)
        )
        generation_provenance = validate_generation_provenance(
            staged_provenance, staged_reproducer
        )
        generation_provenance["provenance_basename"] = (
            generation_provenance_path.name
        )
        generation_provenance["canonical_reproducer"]["basename"] = (
            canonical_reproducer_path.name
        )
        ...
        if common_audit.get("status") != (
            "VERIFIED_BRANCH1_COMMON_RELAXATION_SAT_MODEL"
        ):
            raise AuditError("common-model audit did not return its verified status")
        ...
        seed_rows = read_seed_matrix(staged_matrix, EXPECTED_INPUT_SHA256)
        history_masks, history_info = load_history_exclusion(
            staged_history
        )
        history_info["basename"] = history_exclusion_path.name
        ...
        mask_info["basename"] = masks_path.name
        ...
        augmented = private / "branch1_cegar_augmented.cnf"
        ...
        if (
            cnf_info["sha256"] != FROZEN_AUGMENTED_CNF_SHA256
            or cnf_info["bytes"] != FROZEN_AUGMENTED_CNF_BYTES
            or cnf_info["clauses"] != FROZEN_AUGMENTED_CNF_CLAUSES
        ):
            raise AuditError("augmented CNF differs from the frozen A+ identity")

        stable_output = StableOutputDirectory.create(output_dir)
        try:
            installed_cnf = stable_output.install_file(
                augmented,
                "branch1_cegar_augmented.cnf",
                expected_bytes=FROZEN_AUGMENTED_CNF_BYTES,
                expected_sha256=FROZEN_AUGMENTED_CNF_SHA256,
            )
            if installed_cnf != {
                "basename": "branch1_cegar_augmented.cnf",
                "bytes": FROZEN_AUGMENTED_CNF_BYTES,
                ...
                raise AuditError("installed augmented CNF identity mismatch")
            ...
            base: dict[str, Any] = {
                ...
                "claim_boundary": (
                    "This is one finite CEGAR gate. SAT only authenticates a "
                    "displayed model/witness; UNKNOWN learns nothing; a generated "
                    "UNSAT proof is unchecked until both the augmented proof and "
                    "all four singleton implications are freshly replayed."
                ),
                "historical_solver_only_boundary": (
                    "No earlier extension UNSAT without a proof is imported. "
                    "Such endpoints remain solver-trusted CEGAR evidence only."
                ),
                "inputs": {
                    ...
                    "common_model_assignment_literals": parsed_model[
                        "assignment_literals"
                    ],
                    "universal_bank": {
                        "basename": bank_path.name,
                        "bytes": bank_input["bytes"],
                    ...
                    "fixed_branch1_base_exclusion": {
                        ...
                        "membership_check": (
                            ...
                            "private seed snapshot with edge (97,99) removed"
                        ),
                    },
                    "new_mask_batch": mask_info,
                    "generation_provenance": generation_provenance,
                    "all_path_inputs_staged_before_parsing": True,
                },
                "augmentation": {
                    "positive_edges": [list(edge) for edge in PROVED_POSITIVE_EDGES],
                    "positive_units": list(units),
                    "positive_unit_justification": {
                        "singleton_summary_sha256": (
                            FROZEN_SINGLETON_SUMMARY_SHA256
                        ),
                        "fresh_replay_required_before_unsat_promotion": True,
                    },
                    "new_I18_clauses": len(new_masks),
                    "augmented_cnf": cnf_info,
                    "solver_and_evaluator_input": (
                        "private authenticated snapshot; the public output copy is "
                        "never reopened for solving or SAT-model evaluation"
                    ),
                },
                "solver": {"status": "NOT_RUN"},
                "unsat_promotion": pending_unsat_promotion_record(),
                "runner_provenance": {
                    "current_runner": {
                        "basename": running_script.name,
                        "bytes": runner_input["bytes"],
                        "sha256": runner_input["sha256"],
                        "authenticated_private_snapshot": True,
                    },
                    "pre_hardening_active_exploration_runner": {
                        "snapshot": HISTORICAL_EXPLORATION_RUNNER_SNAPSHOT,
                        "sha256": HISTORICAL_EXPLORATION_RUNNER_SHA256,
                        "canonical": False,
                        "claim_boundary": (
                            "historical solver-trusted exploration only; every "
                            "endpoint must be audited with this hardened runner "
            ...
            if cadical is None:
                base["endpoint_evidence"] = build_endpoint_record(
                    base["solver"], cnf_info
                )
                stable_output.write_json("branch1_cegar_gate.json", base)
                return base

            if _sha256_file(augmented) != FROZEN_AUGMENTED_CNF_SHA256:
                raise AuditError("private augmented CNF changed before solver start")
            raw_proof = private / ".branch1_cegar_unsat.drat.raw"
            raw_solver = run_pinned_cadical(
                cadical, augmented, raw_proof, wall_seconds
            )
            if _sha256_file(augmented) != FROZEN_AUGMENTED_CNF_SHA256:
                raw_proof.unlink(missing_ok=True)
                raise AuditError("private augmented CNF changed during solver run")
            ...
            if state == "SAT_TRANSCRIPT_PENDING_MODEL_AUDIT":
                model_path = private / "branch1_cegar_sat.model.gz"
                try:
                    if not isinstance(internal_stdout, bytes):
                        raise AuditError(
                            "SAT transcript bytes were lost before model audit"
                        )
                    new_assignment, canonical_model = parse_sat_stdout(
                        internal_stdout, PRODUCTION_MAXIMUM_VARIABLE
                    )
                    model_info = deterministic_gzip_bytes(
                        model_path, canonical_model
                    )
                    reparsed, _ = parse_complete_model(
                        model_path, PRODUCTION_MAXIMUM_VARIABLE
                    )
                    if reparsed != new_assignment:
                        raise AuditError(
                            "canonical SAT-model round trip changed assignment"
                        )
                    ...
                    stable_output.install_file(
                        model_path,
                        "branch1_cegar_sat.model.gz",
                        expected_bytes=model_info["bytes"],
                ...
                proof_path = private / "branch1_cegar_unsat.drat.gz"
                try:
                    proof_info = deterministic_gzip_file(raw_proof, proof_path)
                    stable_output.install_file(
                        proof_path,
                        "branch1_cegar_unsat.drat.gz",
                        expected_bytes=proof_info["bytes"],
                        expected_sha256=proof_info["sha256"],
                    )
                except Exception as error:
                    proof_path.unlink(missing_ok=True)
                    solver.update(
            ...
            base["learned_masks"] = []
            base["endpoint_evidence"] = build_endpoint_record(solver, cnf_info)
            stable_output.write_json("branch1_cegar_gate.json", base)
            return base
        finally:
            stable_output.close()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=here / "r3_18_budget7_branch1_cegar_Aplus_provenance.json",
    )
    parser.add_argument(
        "--canonical-reproducer",
        type=Path,
        default=here / "generate_r3_18_budget7_branch1_cegar_Aplus.py",
    )
    parser.add_argument(
        "--matrix",
        type=Path,
    ...
    protected = {
        ...
        args.generation_provenance.resolve(),
        args.canonical_reproducer.resolve(),
        args.matrix.resolve(),
    ...
    try:
        result = run_gate(
            ...
            generation_provenance_path=args.generation_provenance.resolve(),
            canonical_reproducer_path=args.canonical_reproducer.resolve(),
