#!/usr/bin/env python3
"""512-bit author outer checker for the exact-diagonal next candidate."""
...
def main() -> None:
    ...
    exponent_gap = q(34_754, 10_000_000_000)
    ...
    previous_gap = q(34_727, 10_000_000_000)
    previous_base = (u_one - previous_gap).exp()
    ...
    safe_decimal = q(safe_f.numerator, safe_f.denominator)
    previous_safe = q(previous_safe_f.numerator, previous_safe_f.denominator)
    rounding_margin = safe_decimal - base
    safe_improvement = previous_safe - safe_decimal
    assert actual_improvement >= q(1, 100_000_000)
    assert rounding_margin >= q(1, 1_000_000_000)

    print("PASS: 512-bit exact-diagonal retained-spine candidate")
    ...
    print(f"previous_base_lower: {previous_base.lower()}")
    print(f"actual_base_improvement_lower: {actual_improvement.lower()}")
    print(f"safe_decimal_upper: {safe_decimal}")
    print(f"rounding_margin: {rounding_margin}")
    print(f"safe_improvement: {safe_improvement}")
