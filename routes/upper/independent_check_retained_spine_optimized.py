#!/usr/bin/env python3
"""Independent 448-bit Arb replay for optimized retained-spine parameters.

The already refereed transfer theorem defines

    C_* = max_[0,1]^2 min(D, B).

For the exact parameter tuple

    eta=.02863, p=.47136, delta=.000074,
    lambda_0=12000, tau=.0001473,

this script proves the two-sided numerical bracket

    U(1) - 0.000000714 <= C_* <= U(1) - 0.0000007.
...
def main() -> None:
    ...
    eta = arb("0.02863")
    probability = arb("0.47136")
    book_delta = arb("0.000074")
    lambda_zero = arb("12000")
    tau = arb("0.0001473")
    upper_gap = arb("0.0000007")
    lower_gap = arb("0.000000714")
    witness = arb("0.000007069")
    ...
    # All source-level book gates, including the degree slack needed after
    # regularization.  These are part of the new parameter theorem, not
    # merely construction diagnostics.
    assert ZERO < eta
    ...
    assert tau * (arb("0.5") - eta - probability) > ZERO
    ...
    # The coordinate ratio is monotone: it decreases in x and increases in
    # y.  Although x may reach r_diag on the diagonal, y-x is at most r_axis.
    # Hence its exact extrema are the diagonal endpoint and the axis endpoint.
    ...
    decimal_upper = arb("3.780695781309")
    ...
    print("PASS: independent optimized retained-spine replay")
