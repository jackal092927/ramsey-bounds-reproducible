...
\begin{table}[ht]
\centering
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}Xrrrrr>{\raggedright\arraybackslash}X@{}}
\toprule
Active vector & degree & vertices & \(\dim V\) & \(\dim Q\) & depth \(p_a\) & certificate mod \(p\) \\
\midrule
\(\ket0-\ket1\) & 1 & 16 & 2 & 8 & 3 &
\(\det G=512\) \\
\(\ket{00}-\ket{11}\) & 3 & 33 & 4 & 112 & 5 &
\(\det G=655970\) \\
\(\ket{01}-\ket{10}\) & 3 & 33 & 4 & 112 & 5 &
signed relabeling \\
\(\ket{00}-\ket{10}-\ket{11}\) & 3 & 41 & 4 & 176 & 5 &
\(\det G=797443\) \\
three remaining gain/loss vectors & 3 & 41 & 4 & 176 & 5 &
signed relabelings \\
\bottomrule
\end{tabularx}
\caption{Exact active-palette data.  Vertex counts include the \(7m_a\)
register vertices of the active bowties.  The depth column is the largest
number of private vertices in a simplex of the certified filling
(\cref{def:depth}); the basis cone has depth \(1\).  In each determinant row,
