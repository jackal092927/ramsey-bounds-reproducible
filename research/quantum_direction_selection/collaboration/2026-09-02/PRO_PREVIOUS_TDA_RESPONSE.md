# Completed prior TDA Pro response

Collected September 2, 2026 PDT from the registered personal Pro conversation [Audit Quantum Persistence Corollary](https://chatgpt.com/c/6a98c05a-ecf4-83e8-b356-4e20dc3fde3f). Visible account identity matched the local channel registry; UI showed a completed answer, “Worked for 51m 8s”. The earlier packet, not the new round-2 exact-kernel proof, was its input.

This is an advisory model response, not an adopted proof or verified priority claim. Text was extracted from the rendered response DOM; displayed LaTeX was preserved from the equation elements and citation links from their visible anchors. Formatting is reconstructed; multi-source citation pills may expose only their first link. No sidebar, account settings, or hidden conversation state is included.
...
Gyurik et al. allow every simplex to have an arbitrary fixed vertex ordering and then define the guide as a positive uniform subset state in that oriented simplex basis. But the formal problem input lists the two graphs, the dimension, and a compact subset description; it does not explicitly list an orientation oracle.  [source](https://arxiv.org/html/2410.21258v2)
...
for a sufficiently small constant $c$ and a spectral lower bound $g$.  [source](https://arxiv.org/html/2311.17234v2)
...
This is exactly the integer-ratio strategy Hayakawa uses in Section 6.3.  [source](https://arxiv.org/html/2608.02726v1)
...
each $O(g_{0})$ after reducing the fixed constant. Thus the proof works with a known lower bound $g_{0}$, which makes the reduction uniform.  [source](https://arxiv.org/html/2410.21258v2)
...
R\succeq f_{\min}I.$
 ...
 [source](https://arxiv.org/html/2608.02726v1)
...
The weighted source has register vertices of weight $1$ and gadget vertices of weight $\lambda$. Its guide is supported entirely on register simplices and has a succinct Cartesian-product subset description.  [source](https://arxiv.org/html/2410.21258v2)
...
Gyurik et al. explicitly listed unweighted Harmonic Persistence as future work, while Hayakawa explicitly says that his paper only handles the single-complex clique-homology reduction and leaves weighted or filtered extensions open. So the complexity corollary appears new as a written result, but the mechanism is still a short synthesis of the two papers rather than a new spectral construction.  [source](https://arxiv.org/html/2410.21258v2)
...
Mémoli–Wan–Wang define the persistent Laplacian for $K\hookrightarrow L$ and prove that its nullity equals the persistent Betti number; they also study its behavior over filtrations.  [source](https://arxiv.org/html/2012.02808v1)
...
The standard persistent-Laplacian literature proves that its kernel recovers persistent Betti numbers and develops Schur-complement, functoriality, monotonicity, and stability results.  [source](https://epubs.siam.org/doi/10.1137/21M1435471)

Hayakawa proves the single-complex block reduction and explicitly says filtered extensions are not treated.  [source](https://arxiv.org/html/2608.02726v1)
