...
\cref{eq:lambda-E-choice}.  For each endpoint let \(s_i\) be the least
integer with \(2^{-s_i}\le\frac12c_2\eta_0^2g_i\lambda^{26}\), and output
\(E_i=2^{-s_i}\), which satisfies \cref{eq:lambda-E-choice}.  Thus
\(b=O(\log t_{\max})\) and \(s_i=O(\log t_{\max}+\log L)\), so the binary
