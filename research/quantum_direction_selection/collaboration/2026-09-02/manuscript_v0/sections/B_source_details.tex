...
\(A_{\rm in}\) is the sum of the clean-input penalty projectors.  It vanishes
on \(C\) and satisfies \(A_{\rm in}|_{C^\perp}\succeq I\), where
\(C=\ker A_{\rm in}\) is the valid clean-input space.  Split every \(\xi_t\)
into \(C\) and its orthogonal complement.
