...
The source-interface milestone is complete. The immediate bounded milestone is now a primary-source novelty audit centered on the all-geometric-chain concentration estimate over degenerate logical kernels, the resulting inverse-polynomial gap above the entire geometric kernel, the exact natural filtered rank, and their composition with the non-all-or-none BQP1 source. Exact multiplicity alone, the normalized-persistence definition, quotient dimension counting, ordinary min-max, and the already sourced local gadgets are excluded from the novelty claim. The audit must also decide how seriously padding-generated denominator growth limits the conceptual contribution.
