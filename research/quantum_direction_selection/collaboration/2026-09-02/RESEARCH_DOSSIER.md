...
## 9. Completed novelty gate and manuscript transition
...
The equation-level local comparison in [KING_COROLLARY_BOUNDARY_2026-09-03.md](KING_COROLLARY_BOUNDARY_2026-09-03.md) shows that the King--Kohler proof yields the qualitative degenerate-kernel theorem by a short final projection argument if its all-chain estimates are uniform. The completed [Pro source audit](PRO_NOVELTY_REVIEW_2026-09-03.md) and [independent disposition](PRO_NOVELTY_DISPOSITION_2026-09-03.md) agree. The surviving technical candidate is narrower and sharper: a finite-certificate proof that avoids the growing-rank basiswise-perturbation inference and the padded coordinate-bulk obstruction, while allowing the gadget scale to be chosen independently of the logical gap and producing a final gap linear in \(g\).

No mathematical counterexample was found under the explicit finite-palette, exact-filling, private-support, all-bidegree-padding, interface, and independent-interior hypotheses. The quotient route is a meaningful restricted bypass of Lowe et al.'s compatible-harmonic-isometry condition, while remaining standard algebra once the transfer theorem closes the endpoint kernels. The common-copy unweighting result is accepted as a direct Hayakawa corollary, with common labeled blocks and orientations supplying filtration naturality. Padding-generated denominator growth remains an impact weakness; the primary theorem instead uses fixed \(\beta_d(X_{\rm in})=8\) and persistent ranks six versus one.

The resulting [manuscript v0](manuscript_v0/README.md) is now a modular 19-page draft. It leads with the finite-certificate all-chain theorem, then derives exact whole-kernel multiplicity, quotient naturality, gate-dependent \(\mathsf{BQP}_1^{G_2}\) hardness, and the sourced unweighted corollary. It explicitly withholds ordinary \(\mathsf{BQP}\), unrestricted \(\mathsf{SDQC}_1\), gate-independent \(\mathsf{BQP}_1\), complex phases, intrinsic denominator growth, practical gap claims, and first-quantum-persistence language.

The remaining source gate is exact and narrow: obtain the detailed final SIAM King--Kohler text and compare its padded-bulk argument with the outside-harmonic/fixed-local repair. If it already proves an equivalent uniform \(t\lambda^2\) leakage estimate, analytic novelty drops sharply. This affects positioning and venue, not the decision to maintain a complete scoped draft.
