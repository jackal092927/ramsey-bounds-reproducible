...
\(u'>u\) and \(v'>v\). Its defining bound is stronger than (1.1).
\(\square\)
...
a common blue page \(T\) satisfying (3.1). \(\square\)
...
Exponentiation and the remaining factor \(1-\mu\) prove (4.1).
\(\square\)

\subsection{Proof of Theorem A}

The strict inequality in (A.1) implies \(x_0+\mu_0<1\). By Lemma 4
...
A single sufficiently large \(L_0\) makes (5.6) true for every
\(n\ge2\).
...
(5.5) does not decrease. The positive right side prevents deletion of
...
Lemma 1 and the interior condition in (5.2) imply, for sufficiently
large \(L_0\), that if
...
Otherwise, using \(d+\delta_n-p\le1\) in (5.5),
...
Since (5.8) is exponential in \(n+\ell\), increasing \(L_0\) once makes,
...
Lemma 2 and (5.10)--(5.12) show that the contribution from \(W\) is at
most
...
\(\alpha_B\) need not be defined. By (5.7),
\(|Y'|\ge(p-\varepsilon)|Y|\). Also, (5.4) and (5.13) give
...
condition (5.16) and (5.5) show that \((X_R,Y')\) satisfies the
...
Equations (5.8) and (5.14), after one final uniform increase of \(L_0\),
give
...
But \(x=x_0+\varepsilon\) and \(\mu=\mu_0+\varepsilon\), so (5.21)
strictly contradicts the final inequality of (5.2). The induction, and
...
\item
  ...
  printed inequality.
\item
  The concavity calculation supplies an upper bound in (5.19), and the
...
numerical inequalities. A separate adversarial replay of this standalone
proof is still required before changing the public claim label.
