...
Assume in addition that \(U\ge0\).  Suppose (1.1) holds uniformly for
...
(1.1).  For \(x\ge y\), put \(k'=\lfloor xk\rfloor\) and
...
\cref{app:upper-certificate}.  We state the complete specialization used
...
\[
 ...
 \tag{2.2a}\label{eq:correlation-definition}
\]
...
\[
 ...
 \tag{2.7}\label{eq:correlation-input}
\]
\end{assumption}

The point of stating \cref{ass:yang-mao} is auditability.  The correlation
property in (2.7) is proved in this paper; the regularization and book
...
\[
 ...
 \tag{2.8}
\]
Here \(q\) and \(\Xi\) are the explicit quantities in (2.3)--(2.4).  Define
\begin{align}
 M_\tau(\sigma)=\max\{&
 \wideU((1-\sigma_R-\tau)_+,1-\sigma_B),\nonumber\\
 &\wideU(1-\sigma_R,(1-\sigma_B-\tau)_+)\},\tag{2.9}\\
 P(\sigma)&=c_\eta(\sigma_R+\sigma_B)+\tau q+M_\tau(\sigma),\tag{2.10}\\
 Q(\sigma)&=c_\eta(\sigma_R+\sigma_B)+2\tau\Xi,\tag{2.11}\\
 B(\sigma)&=\max\{P(\sigma),Q(\sigma)\}.\tag{2.12}
\end{align}
Finally set
\[
...
nothing to prove.  Otherwise write \(s_i=\sigma_i k+O(1)\).  From (2.5),
\[
...
possible book colour, using (2.2) and (2.9); the latter supplies the common
...
exponent inequalities give exactly (2.6).  Since \(p<1/2-\eta\) and the
