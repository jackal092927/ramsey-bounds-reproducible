\section{The exact-diagonal correlation certificate}\label{sec:exact}
...
\begin{align}
 P(u)&=\frac{e^u+2e^{-u/2}\cos(\sqrt3u/2)}3,\tag{4.1}\\
 N(u)&=\frac{e^{-u}+2e^{u/2}\cos(\sqrt3u/2)}3.\tag{4.2}
\end{align}
...
\begin{lemma}[Half-line ratio]\label{lem:ratio}
For every \(u\ge u_0\),
\[
...
\(aB_T-9(T-1)>0\) and the separate base gate
\[
 D(2.9)>506.45>0.
\]
Moreover
...
\]
Indeed, for \(u\ge2.9\) one has \(e^u-T>1\), and therefore
\[
D'(u)>4e^u-e^{u/2}>0.
\]
Thus \(D(u)>0\) and then \(B_T'(u)>0\) throughout \([2.9,\infty)\).
Together with the endpoint gate, this proves that \(B_T\), and therefore
\(R-T\), remain positive.
\end{proof}

\begin{lemma}[Separator]\label{lem:separator}
...
\[
 ...
 \tag{4.7}
\]
holds for every real \(y\).  By \cref{lem:ratio}, \(R>2+2\sigma_0\),
whereas \(A,H(y),H(-y)\ge1\).  Thus the right side of (4.7) is less than
\(-(1+2\sigma_0)/2=-\sigma_*\).  Exchange the coordinates if necessary.
\end{proof}

\subsection{Complete two-dimensional reduction}

\begin{lemma}[Diagonal reduction]\label{lem:diagonal-reduction}
...
\begin{lemma}[Exact diagonal envelope]\label{lem:envelope}
For every \(u\ge0\),
\[
...
On each cell a 512-bit Arb evaluation encloses the left side of (4.11); the
...
\[
 ...
 \tag{4.12}
\]
Both terms decrease.  At \(u=20\), (4.12) is below
...
Combining \cref{lem:diagonal-reduction,lem:envelope}, if
\(A_1,A_2\ge-1\) and \(M=\max(A_1,A_2)\), then
\[
 F(LA_1,LA_2)<D_0\exp\!\left(4u_0(M+1)^{1/3}\right).
 \tag{4.13}
\]
If \(M\le0\), the left side is nonpositive by the sign case in
\cref{lem:diagonal-reduction}, whereas the right side is positive.  If
\(M>0\), take \(u=u_0M^{1/3}\); then the diagonal-reduction parameter is
\(m=LM=u^3\) and the \(w\) in \cref{lem:envelope} equals
\(u_0(M+1)^{1/3}\), proving the displayed inequality.
...
\begin{lemma}[Positive-moment expectation]\label{lem:positive-expectation}
...
\[
 ...
 \tag{4.14}\label{eq:positive-expectation}
\]
\end{lemma}
...
\[
 ...
 \tag{4.15}\label{eq:tail-budget}
\]

\begin{proposition}[Certified correlation tail]\label{prop:correlation-tail}
...
\begin{proof}
Fix data from the definition \eqref{eq:correlation-definition}, put
\(\mathcal E=\{Z_1\ge-1,Z_2\ge-1\}\),
\(M=\max\{Z_1,Z_2\}\), \(Y=((M+1)_+)^{1/3}\), and
\(c=4u_0\).  By \cref{lem:positive-expectation,lem:separator} and
\eqref{eq:positive-expectation}, while (4.13) controls the good event,
\[
 0\le\mathbb EF(LZ_1,LZ_2)
 <D_0\mathbb E(e^{cY}\mathbf1_{\mathcal E})
   -\sigma_*\mathbb P(\mathcal E^{\mathrm c}).
 \tag{4.16}\label{eq:master-tail}
\]
If \(\mathbb P(\mathcal E)\ge\beta\), choose \(\lambda=-1\) and either
coordinate in \eqref{eq:correlation-definition}.  Otherwise suppose, for a
...
\begin{align*}
 ...
 =\beta(1+2/\varepsilon).
\end{align*}
Substitution into \eqref{eq:master-tail} makes its right-hand side at most
\(D_0\beta(1+2/\varepsilon)-\sigma_*(1-\beta)<0\), by
\eqref{eq:tail-budget}.  This contradiction proves the proposition.
\end{proof}
...
\[
 ...
 =9.8930786017752,
\]
which is the exact handoff asserted in \eqref{eq:correlation-input}.
