\section{Pinned source interface and quantifier ledger}\label{app:bridge}
...
from the locally proved weighted reverse-propagation lemma and imports neither
that lemma, the first-moment conversion, nor the Ramsey conclusion proved in
\cref{sec:rate-lower}.
...
these give (2.4)--(2.6) and \cref{lem:mills}.\\
HMS 1192--1289&
The red law of total probability, cutoff conditioning, projection merge, and
scalar reverse-induction allocation.  Equation (2.36a) is the frozen scalar
baseline.  The indicator-kernel conjugation and weighted propagation (S3) are
proved locally from (2.33), (2.39), and (2.40); neither the nonuniform
one-step estimate nor its propagation is attributed verbatim to HMS.\\
HMS 385--410 and 955--971&
For every specified retained set, the red-perfectness failure factor is
\((p/10)^{10\ell u}\).  This is the sole external extraction input (2.41);
...
extraction argument, yielding the raw probability inequality (2.42).  The
...
\tag{B.4}
\]
The blue interface is the explicit probability estimate (2.42).  Dividing
...
The absolute constant \(K\) in \eqref{eq:source-K} is fixed before selecting the
large-\(C\) threshold, but its numerical value is not provided by the pinned
source.  At theorem level the quantifiers therefore begin in the order
\[
 \text{fix }K,\qquad
 \exists C_0=C_0(K)\ \forall C\ge C_0\ \exists\ell_0(C)\
 \forall\ell\ge\ell_0(C),
\tag{B.7}
\]
followed by \(\ell\to\infty\).  Within the proof, the auxiliary \(w\) is
fixed in an admissible right-neighborhood of \(\omega_0\) during this limit
and is then sent down to \(\omega_0\).  Only after that fixed-\(C\) theorem is
established is \(C\to\infty\) used to obtain the coefficient \(1/64\).
No explicit value of \(C_0(K)\) or \(\ell_0(C)\) is asserted.

The resulting theorem is source-relative: its local residual algebra,
three-factor bridge, weighted propagation, reverse-induction charge,
extraction sum, first moments and red--blue crossing are proved here.  Items
(S1), (S2), (S4), and (S5) are version-pinned source hypotheses; (S3) is
\cref{lem:weighted-reverse}, proved locally rather than attributed to either
source.  The arithmetic checker corroborates the finite formulas but does not
replace the four imported probabilistic interfaces or the local proof.
