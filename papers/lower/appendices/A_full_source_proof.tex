...
Decompose the centered exponent into the source uniform part, the weighted
increment, and the remaining factor.  Use conjugate exponents
\[
 Q_0=\frac d{4t_0k},\qquad Q_1=\frac d{4R_i},\qquad
 P^{-1}=1-Q_0^{-1}-Q_1^{-1}. \tag{A.15}
\]
The source square-moment estimate is applicable because
\[
 \frac{4m_0}{D}+\frac{4a_+}{D^2}<\frac12. \tag{A.16}
\]
The weighted quadratic factor is bounded by
\[
 \frac{16}{3}\frac{S_i}{d}. \tag{A.17}
\]
The three changes in the linear cumulant comparison are, respectively,
bounded by
\begin{align}
 &\frac{4m_0M_w^2}{D}\frac{S_i}{d}, \tag{A.18}\\
 &\frac{2M_w^2a_+}{D^2}\frac{S_i}{d}, \tag{A.19}\\
 &\frac{32m_0^2M_w^2a_+}{a_-D^2}\frac{S_i}{d}. \tag{A.20}
\end{align}
For (A.20), one first uses the row-sum bound $R_i$, then
$\sum_j(u_j^0)^2\le2m_0^2M_w^2S_i/a_-$; this is precisely where the
ratio $R_i/S_i$ in (A.12) enters.  Adding (A.18)--(A.20) gives
$\widehat L_w(C)S_i/d$.  The central deterministic gain is
$m_0^2S_i/d$, and hence the net increment is
\[
 -\left(m_0^2-\frac{16}{3}-\widehat L_w(C)\right)\frac{S_i}{d}
 =-\widehat B_w(C)\frac{S_i}{d}. \tag{A.21}
\]

The pre-existing exact-CGF term $G_*(C)$ compares the uniform $t_0$
centered component with the source allocation.  Equations
(A.17)--(A.21) concern only $\Delta=T-t_0$ and the change in H\"older
exponent.  Therefore $G_*(C)$ and the new term are disjoint ledger entries.

\subsection{One-step conclusion and extraction}

Combining (A.10), (A.14), and (A.21) gives the empty-state bound
\[
 P^*_{R,r}\le\mathcal H_C(r)
 \exp\!\left[-\mathcal D_{C,w,d}(r)
 -\frac1d\left\{2\widehat B_wA_0\binom r4+E_wK_r\right\}
 +J_{r,d}\right], \tag{A.22}
\]
where
\[
 0\le J_{r,d}\le
...
then the loss in the new deterministic exponent is at most
\[
 \frac1d\left[
...
\tag{A.24}
\]
The bracket is $O(1/\log C)<1$.  The unchanged source failure charge is
exponential in $-\ell u$ with a larger absolute constant, so the sum over
deleted subsets is $o(1)$.  This proves that (A.22) survives the same
perfect-sequence extraction as the pinned source theorem.
