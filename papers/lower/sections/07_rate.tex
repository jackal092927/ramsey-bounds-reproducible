...
\label{sec:rate-lower}

\subsection{The two explicit first-moment thresholds}

Fix \(C\) and \(w>\omega_0\), and let \(\ell\to\infty\).  From (2.12),
(2.18), (6.1), and (6.12),
\[
\begin{aligned}
...
For \(q_\ell=\lfloor C\ell\rfloor\), (2.42) and
...
\(\sigma\) strictly below the minimum of the right sides of (7.2) and
(7.4).
...
of (7.2) over \(w>\omega_0\), using continuity as
\(w\downarrow\omega_0\), and then using (7.9) proves
\cref{thm:main-lower}.
...
\begin{definition}[Admissible scalar residual family]\label{def:admissible}
...
\tag{7.15}
\]
For \(r\ge2\), writing \(F_i^T\) for (3.1) with coefficient array \(T\), the
...
not listed in (7.17).
\item Relative to the exact weighted rule (S3), the family's credited
...
\begin{proposition}[Cap within \cref{def:admissible}]\label{prop:method-cap}
For every fixed \(M_0,M_{\rm ext}\), the leading functional in (7.21)
satisfies
\[
...
\(\beta\) and \(\mu\).  The exact constraint (7.16) gives
\[
 \beta\le m_0^2.
\tag{7.24}
\]
At \(b=c\mathbf1\), the row condition (7.15) and the diagonal formula (A.1)
...
Let \(\mathcal L\) be the feedback operator in (7.14).  Its row norm is at
...
by (7.13).  The exact exposure multiplicity is
\[
 \sum_{i=1}^{r-1}S_i
 =\sum_{1\le j<q\le r}(j-1)\Delta_{jq}.
\tag{7.28}
\]
The first term of (7.27) contributes
\[
 2A_0\binom r4[1+O(\rho)]
\tag{7.29}
\]
to (7.28).  The last term changes the numerator in (7.19) by at most
...
\tag{7.30}
\]
Summing (7.30) gives \(\mu\beta^2K_r/2\).  Set \(r=\ell\), divide (7.19)
by \(\ell^2\), and use
\(\binom\ell4/\ell^4\to1/24\), \(K_\ell/\ell^4\to1/12\), and
\(d/\ell^2\to D^2\).  Equations (7.24)--(7.30) yield
\[
 \operatorname{Lead}_C(\mathcal E)
...
and then the limsup proves (7.22).

For \(e_{ij}=\theta(i-1)\), equations (5.5), (7.7), and (7.8) make
(7.30) asymptotically tight.  Equations (5.11), (6.2), and (4.4) verify
(7.15)--(7.18), and (7.13) holds with \(M_0^*=1\).  To verify (7.20), write
\(\Delta^{(r)}\) for the array rebuilt at target \(r\), and put
\(r_-=\ell-u\).  Subtracting the two positive recursions (7.14), and using
...
\tag{7.32}
\]
Indeed, the second bound is (5.1).  If \(U\) denotes the first maximum,
subtraction of (7.14) gives
\(U\le A_0u+\rho U+\gamma_duK_2m_0^2\ell\).  Now
\(A_0=O(m_0^2)\), \(\gamma_d\ell\le\rho\), and \(\rho<1/2\) give the first
bound.  The multiplicity identity (7.28) and (5.4) now give
\[
...
\(m_0^4/D^2=O(1/\log C)\), (7.33) proves (7.20) for some absolute
\(M_{\rm ext}^*\).  Finally, the functional is exactly (2.23) up to
\(o(1/\log C)\), and (7.12) gives the asserted limit (7.23).
\end{proof}
