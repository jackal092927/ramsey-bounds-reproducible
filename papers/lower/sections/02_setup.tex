...
HMS v2 and Lin--Niu v2 sources identified in Appendix~\ref{app:bridge}.
...
Only the implication (2.37)\(\Rightarrow\)(2.38) is the additional hypothesis
in (S3).  The following source identity and deterministic projection algebra
record what that implication must preserve; they neither add a second
hypothesis nor prove (S3).
...
\tag{2.40}
\]
Equations (2.33), (2.39), and (2.40) record the algebra that the weighted
extension in (S3) is required to preserve.  The rule propagates (2.38) by
reverse induction from \(s=r\).
...
\eqref{eq:scalar-baseline}; the only additional source-side hypothesis is the
precisely delimited weighted rule (S3).
\end{assumption}
