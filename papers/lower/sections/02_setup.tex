...
\tag{2.36}
\]
The positive-direction CGF is exactly (2.4).  In the source pre-Cauchy
allocation, its centered-linear proxy is precisely
\(\sum_{j>i}(u_j^0)^2\), with \(u_j^0\) as in (5.9); replacing that proxy
by the exact CGF produces the same-history deficit (5.16).
...
extension in (S3) is required to preserve; they do not independently discharge
that extra hypothesis.  The rule in (S3) propagates (2.38) by reverse induction
...
\tag{2.41}
\]
Equation (2.41), rather than a final Ramsey bound, is the scalar source input.
No nonuniform one-step estimate is imported.  Sections
\ref{sec:residual}--\ref{sec:induction} compare the nonuniform kernel with
(2.41) and prove (2.37) for the array used here.
...
The constant \(K\) in (2.10) is not effective in the pinned source.
