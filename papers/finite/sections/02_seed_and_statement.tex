...
\paragraph{Evidence grade and logical role.}
The assertion \(\alpha(H)<18\) is an exact deterministic trusted-code result:
the frozen matrix and inspectable bitset search reproduce it, and exhaustive
small-instance comparisons test the pruning invariant.  It is not backed by a
DRAT/LRAT trace or a proof-assistant certificate.  This assertion describes
why \(H\) is a near miss, but it is not a logical premise of
\cref{thm:main}.  The deletion-barrier proof uses the fixed edge set and seed
triangle of \(H\), while the independence-number hypothesis is imposed on the
putative final graph \(F\).  Thus the implication proved below does not infer
anything from \(\alpha(H)<18\).
