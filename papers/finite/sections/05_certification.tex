\section{Proof production, checking, and trust}\label{sec:certification}
...
\]
All three checks terminated with exit code zero and reported an exact
\texttt{s VERIFIED} status line.

\begin{table}[t]
\centering
\caption{Exact-six proof artifacts and replay scale.  Hashes are abbreviated
here and given in full in \cref{tab:budget6-artifacts}.}\label{tab:proof-scale}
\begin{tabular}{@{}crrrl@{}}
\toprule
...
\item regenerate the production PySAT counter and, in a separate checker that
  does not import \texttt{CardEnc}, independently generate the
  sequential-counter schema and compare its variables and clauses in order;
...
\begin{itemize}
\item the bitset independent-set verifier used for the descriptive seed fact
  \(\alpha(H)<18\), which is not a premise of the deletion-barrier implication;
\item the \texttt{drat-trim} proof kernel, together with its compiler and
  execution hardware; and
\item the independently inspectable scripts that map matrices, sequential
  schemas, and masks to DIMACS.
\end{itemize}
...
exhaustive small-instance projection tests for the independent counter schema,
constructive auxiliary assignments, complete clause reconstruction, content
hashes, gzip integrity tests, and proof replay with a freshly compiled checker.
PySAT remains production provenance, but
\cref{lem:seqcounter-projection} and the \texttt{CardEnc}-free comparison mean
that its counter semantics is no longer assumed merely because PySAT emitted
the bytes.  A future LRAT conversion and formally verified LRAT checker could
reduce the trusted kernel further, but is not required for the conventional
