...
\cite{JainLiRobereXun2024} yields at multiplicity two
(\cref{prop:quantum-lower}, stated for order $\lfloor n/2\rfloor$ and hence
valid here).}
\label{tab:bounds}
\end{table}
...
and shows that the cited ingredients compose to an exponent of at most
$1/12$, and at most $1/24$ under the parameter condition as printed, with
equality at multiplicity two; the resulting
$\Omega(N^{1/12})$ bound (\cref{prop:quantum-lower}) is, to our knowledge,
the best known quantum lower bound.
...
$2\cdot2^{-\binom K2+c(c-1)}(2T)^{K-c}$ for every $c\le K/2$; optimizing $c$ gives
