...
We use the elementary $4^{K-1}$ sufficient threshold because it gives the
recursive size invariant underlying this construction.
...
At the succinct Ramsey scale, \cref{thm:main} reduces the query count of the
standard constructive recursion from $O(N)$ to
$O(\sqrt N\log N\log(\log N/\eta))$.  The randomized classical lower bound in
\cref{prop:classical-lower} provides a complementary benchmark in the same
parameterization.

A closely related exact-model analysis reports an $N^{1-o(1)}$ quantum-query
lower bound for its default Ramsey relation
\cite[Section III, p.~415]{JainLiRobereXun2024}.  Under its formal
$N=2^n$, $K=n/2$ convention, this creates an apparent discrepancy with
\cref{cor:succinct}.  Section~\ref{sec:prior-art} aligns the oracle models and
examines the parameter substitution underlying the reported exponent.
...
the random-Painter theorem, and compares the result with the closest related
work before presenting future directions.
