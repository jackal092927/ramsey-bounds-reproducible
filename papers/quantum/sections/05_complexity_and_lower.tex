...
The result counts queries, not total gates or data-loading time.  Every
search starts from the uniform state on the fixed power-of-two universe
$[M]$, not from a pre-prepared state on the unknown $S_i$.  The $O(K)$
measured pivot labels and colours specify the reversible membership tests.
No QRAM data structure is assumed, but coherent edge access is: if one
edge-oracle evaluation costs $C_G$ gates, its contribution is incurred for
each oracle call, not once for the whole algorithm.  Constructing such an
oracle from an explicit adjacency matrix is outside our query bound.
...
bound of Impagliazzo--Naor, as recorded in this parameterization by
Komargodski--Naor--Yogev
\cite{ImpagliazzoNaor1988,KomargodskiNaorYogev2019}.  Therefore no
...
\cref{app:multicolour}; this extension is not needed for the sharper binary
theorem.
